...
with tab1:
    ...
    if process_clicked:
        ...
        for doc_name, chunk_dict in doc_chunks_map.items():
            ...
            # Download button for this file's chunks
            st.download_button(
                label=f"Download All Chunks for {doc_name} (JSON)",
                data=json.dumps(chunk_dict, indent=2),
                file_name=f"{doc_name}_all_chunks.json",
